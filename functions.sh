...
provision_k8s_baremetal() {
    ...
    if [[ "$DEPLOYMENT" == "k8s" ]]; then
        ...
        ansible_cmd+="yum -y remove python-netaddr; pip install --upgrade pip; pip install ansible==2.9.20; ansible-playbook -i /bmra/inventory.ini /bmra/playbooks/k8s/patch_kubespray.yml;"
    fi
...
-v ~/.ssh/:/root/.ssh/ rihabbanday/bmra21.03-install:centos \
/bin/bash -c "pip install ansible==2.9.20; ansible-playbook -i /bmra/inventory.ini /bmra/playbooks/${BMRA_PROFILE}.yml"
}

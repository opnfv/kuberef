...
provision_k8s() {
    ...
    if [[ "$DEPLOYMENT" == "k8s" ]]; then
        ...
        ansible_cmd+="pip install --upgrade pip==9.0.3; yum -y remove python-netaddr; pip install netaddr==0.7.19 ansible==2.9.6; ansible-playbook -i /bmra/inventory.ini /bmra/playbooks/k8s/patch_kubespray.yml;"
    fi

...
copy_files_jump() {
    scp -r -o StrictHostKeyChecking=no $CURRENTPATH/{hw_config/,sw_config/,setup_network.sh,httpboot/deployment_image.qcow2} \
...
# Install and run cloud-infra
    ...
    cd $PROJECT_ROOT/engine/engine && git checkout 863cf75
    ./deploy.sh -o centos7 -p file:///$PROJECT_ROOT/engine/engine/pdf.yaml -i file:///$PROJECT_ROOT/engine/engine/idf.yaml -l provision
EOF
}

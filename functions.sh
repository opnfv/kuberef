...
clean_up() {
    ...
    sudo rm -rf /var/lib/libvirt/images/$VM_NAME --managed-save
}
...
copy_files_jump() {
    scp -r -o StrictHostKeyChecking=no $CURRENTPATH/{hw_config/$VENDOR/,sw_config/$INSTALLER/,setup_network.sh,httpboot/deployment_image.qcow2} \
...
# Install and run cloud-infra
    if [ ! -d "${PROJECT_ROOT}/engine" ]; then
      ...
      cp $PROJECT_ROOT/$VENDOR/* ${PROJECT_ROOT}/engine/engine
     ...
     cd ${PROJECT_ROOT}/engine/engine && ./deploy.sh -s ironic -d centos7 -p file:///${PROJECT_ROOT}/engine/engine/pdf.yaml -i file:///${PROJECT_ROOT}/engine/engine/idf.yaml
EOF
}
